...
def my_project_dashboard_function(projects, session_data):
    try:
        ...
        try:
            ...
            # Join inventory with projects_managers, managers_data, and user_info
            # First try to match Owner with managers_data.ID, then with user_info.ID
            query = f'''
            SELECT i.*, pm.Projects AS Project, pm.project_id, 
                   COALESCE(md.Name, ui.Name) AS OwnerName
            FROM inventory i
            JOIN projects_managers pm ON i.project_id = pm.project_id
            LEFT JOIN managers_data md ON i.Owner = md.ID
            LEFT JOIN user_info ui ON i.Owner = ui.ID
