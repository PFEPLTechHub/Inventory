        ...
        if (xhr.readyState === XMLHttpRequest.DONE) {
            if (xhr.status === 200) {
                ...
                if (xhr.responseText === "Approval has been successfully given") {
                    ...
                    const eway = formObject.find(obj => obj.EwayBill)?.EwayBill || "";

                    // Debug logs for outbound payload
...
window.onload = function () {
    ...
    xhr2.onreadystatechange = function () {
        if (xhr2.readyState == 4 && xhr2.status == 200) {
            console.log("Received response from /get_form_data");
            ...
            if (transactionDetails.length > 0) {
                ...
                productDetails.forEach(function (row, index) {
                    ...
                    if (senderImage && senderImage.trim() !== "") {
                        ...
                        senderImageCell.textContent = 'No image available';
                    }
                });
            } else {
